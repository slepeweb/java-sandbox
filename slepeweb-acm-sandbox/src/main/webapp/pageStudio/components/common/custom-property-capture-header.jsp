		...
		<script language="JavaScript" type="text/javascript">
			submit_component_config = function (){
				if (pageStudioTool.selectedOverlay != null) {
					...
					{ method: "get",
					parameters: formContents + "&view=Page+Studio&action=getComponent&<%= URL_PARAM_COMPONENT_CONFIGURATION %>=<%= URL_PARAM_STORE_PROPERTIES %>&cid="+encodeURI(cid) + "&cInstId="+encodeURI(cInstId) + "&" + pageStudioTool.addRequestedItemParams(), evalScripts: true });
					
					//reinitialise sortability to include the new component
					pageStudioTool.initFieldSortability();
					return false;
				}
				return false;
			}
