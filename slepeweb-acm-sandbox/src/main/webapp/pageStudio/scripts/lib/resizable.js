...
// Modified for ACM, July 2008
// The following lines, modified for ACM: 138, 149-155
...
Resizable.prototype = {
	...
	initResize: function(event) {
		...
		if(Event.isLeftClick(event)) {
			...
			eleWidth = this.element.getStyle('width');
			if(eleWidth.indexOf('%') != -1){
				eleWidth = this.element.offsetWidth;
			}else{
				eleWidth = parseInt(eleWidth);
			}
			this.size = [eleWidth, parseInt(this.element.getStyle('height')) || 0];
	...
	updateResize: function(event, pointer) {
		...
		var scale = false;
		if (event.ctrlKey && !this.options.constraint) scale = true;
		this.draw(pointer, scale);
	...
	draw: function(point, scale) {
		var p = [0,1].map(function(i){
			if (scale) return this.size[i] + (((point[0] - this.pointer[0]) / this.size[0]) * this.size[i]);
