...
<c:if test="${pager.visible}">
	<p id="advanced-search-pager">
		<c:if test="${pager.previous}"><span 
				class="pager arrow left"><a href="${urlPrefix}/${pager.previousBlock}"><i 
					class="fas fa-angle-double-left" title="Jump left"></i></a></span><span 
				class="pager arrow left"><a href="${urlPrefix}/${pager.previousSelection}"><i 
					class="fas fa-angle-left" title="Previous"></i>Previous</a></span></c:if>
					
		<c:forEach items="${pager.navigation}" var="_option">
			<span class="pager <c:if test="${_option.selected}">selected</c:if>"><a href="${urlPrefix}/${_option.value}">${_option.name}</a></span>
		</c:forEach>
		
		<c:if test="${pager.next}"><span 
			class="pager arrow right"><a href="${urlPrefix}/${pager.nextSelection}">Next<i 
				class="fas fa-angle-right" title="Next"></i></a></span><span 
			class="pager arrow right"><a href="${urlPrefix}/${pager.nextBlock}"><i 
