...
public class SolrServiceImpl implements SolrService {
	...
	public boolean isEnabled() {
		return Util.isPositive(solrIsEnabled);
	}
...
//				LOG.info(String.format("Solr query: [%s]", q.getFilterQueries().toString()));
				...
				try {
					...
					LOG.info(String.format("Query returned %d results [%s]", 
							response.getResults().size(), qr.getHeader().toString()));
					
					response.setPager(new SolrPager<FlatTransaction>(
							response.getTotalHits(), response.getResults(), 
	...
	private List<FlatTransaction> makeDocs(Transaction t) {
